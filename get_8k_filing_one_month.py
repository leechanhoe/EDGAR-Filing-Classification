...
# ─ MongoDB 접속 정보 ─
MONGO_USER = 'opraipb'
MONGO_PASSWORD = 'gmsh_opraipb'
MONGO_HOST = '10.196.10.30'
MONGO_PORT = '27017'
MONGO_DB   = 'edgar'
RAW_COLL   = '8k_filing_raw'
SPLIT_COLL = '8k_filing_raw_split'

mongo_uri    = f"mongodb://{MONGO_USER}:{MONGO_PASSWORD}@{MONGO_HOST}:{MONGO_PORT}/"
mongo_client = MongoClient(mongo_uri)
raw_col      = mongo_client[MONGO_DB][RAW_COLL]
split_col    = mongo_client[MONGO_DB][SPLIT_COLL]
...
def get_recent_8k_filings(cik, ticker, one_month_ago):
    ...
    try:
        ...
        # form 타입, accessionNumber, primaryDocument, filingDate, items를 매칭
...
def process_filings(filings):
    ...
    df = pd.DataFrame(filings, columns=[
        "ticker","accession_number","url","filing_date","item_number"
    ])
    df["content"] = ""
    # 1) 원본 content 추출
    for i,row in df.iterrows():
        print(f"[{i+1}/{len(df)}] Fetching content")
        df.at[i,"content"] = extract_filing_content(row["url"])
    # 2) raw upsert
    ops = []
    ...
    for _,row in df.iterrows():
        items = [x.strip() for x in row["item_number"].split(",") if x.strip()]
        pieces = split_by_items_whitespace_agnostic(
            row["content"], items, item_type_mapping_for_split
        )
        # fallback
        rem = row["content"]
        ...
        fallback = rem.strip()
        for it in items:
            piece = pieces.get(it,"") or fallback
            doc = {
                "ticker":          row["ticker"],
