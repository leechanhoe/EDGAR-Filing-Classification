...
def get_recent_8k_ciks(end_date=None):
    ...
    while True:
        try:
            ...
            # 현재 페이지의 날짜 확인
            date_cells = soup.find_all('td', string=lambda x: x and x.strip().startswith(date_str))
            if not date_cells:
                break
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            debug_df = pd.DataFrame({
                ...
                'Item Numbers': [', '.join(item_numbers)],
                'Original Content': [content_full]
            })
            
            debug_filename = 'debug_before_split.csv'
            if os.path.exists(debug_filename):
                debug_df.to_csv(debug_filename, mode='a', header=False, index=False, encoding='utf-8-sig')
            else:
                debug_df.to_csv(debug_filename, index=False, encoding='utf-8-sig')
            
            # Content를 Item별로 분리
            item_contents = {}
            remaining_content = content_full
            
            # 각 Item 번호에 대해 처리
            for item in item_numbers:
                # Item 패턴 생성 (줄바꿈 다음에 나오는 경우만 매칭)
                item_desc = item_type_mapping_en.get(item, "")
                item_pattern = f'(?:^|\\n)item\\s*{item}\\.?\\s*{re.escape(item_desc)}'
                
                # 해당 Item으로 시작하는 부분 찾기
                match = re.search(item_pattern, remaining_content, re.IGNORECASE | re.MULTILINE)
                if match:
                    start_pos = match.start()
                    
                    # 다음 Item의 시작 위치 찾기
                    next_item_start = len(remaining_content)
                    for next_item in item_numbers:
                        if next_item != item:  # 현재 Item이 아닌 다른 Item 찾기
                            next_desc = item_type_mapping_en.get(next_item, "")
                            next_pattern = f'(?:^|\\n)item\\s*{next_item}\\.?\\s*{re.escape(next_desc)}'
                            next_match = re.search(next_pattern, remaining_content[start_pos + 1:], re.IGNORECASE | re.MULTILINE)
                            if next_match:
                                next_pos = start_pos + 1 + next_match.start()
                                next_item_start = min(next_item_start, next_pos)
                    
                    # 현재 Item의 내용 추출
                    item_content = remaining_content[start_pos:next_item_start].strip()
                    item_contents[item] = item_content
                    
                    # 남은 내용 업데이트
                    remaining_content = remaining_content[next_item_start:].strip()
